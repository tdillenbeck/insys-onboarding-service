...
func (t *TaskInstanceService) UpdateExplanation(ctx context.Context, id uuid.UUID, explanation string) (*app.TaskInstance, error) {
	...
	query := `UPDATE insys_onboarding.onboarding_task_instances SET explanation=$1, updated_at=$2 WHERE id=$3 RETURNING id, location_id, onboarding_category_id, onboarding_task_id, completed_at, completed_by, verified_at, verified_by, button_content, button_external_url, content, display_order, status, status_updated_at, status_updated_by, title, explanation, created_at, updated_at`
