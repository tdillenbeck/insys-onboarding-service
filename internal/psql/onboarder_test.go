...
func TestOnboarderService_ReadBySalesforceUserID(t *testing.T) {
	db := initDBConnection(t)
	clearExistingData(db)
	...
	tests := []struct {
	...
	}{
		{
			name:   "successfully reads an existing onboarder",
			fields: fields{DB: db},
			args:   args{context.Background(), "testing salesforce_user_id"},
			want: &app.Onboarder{
				UserID:                    existingUserID,
				SalesforceUserID:          null.NewString("testing salesforce_user_id"),
				ScheduleCustomizationLink: null.NewString("testing existing_schedule_customization_link"),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := s.ReadBySalesforceUserID(tt.args.ctx, tt.args.salesforceUserID)
			if (err != nil) != tt.wantErr {
				t.Errorf("OnboarderService.ReadBySalesforceUserID() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !cmp.Equal(got, tt.want, opts...) {
				t.Errorf("OnboarderService.ReadBySalesforceUserID(). Diff:  %v", cmp.Diff(got, tt.want, opts...))
			}
		})
	}
}
